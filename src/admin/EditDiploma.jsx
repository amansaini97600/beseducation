...
export default function EditDiploma() {
  ...
  useEffect(() => {
    const fetchDiploma = async () => {
      try {
        ...
        const res = await axios.get(`https://beseducation-backend.onrender.com/diplomas/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      await axios.put(`https://beseducation-backend.onrender.com/diplomas/${id}`, updatedData, {
        headers: { Authorization: `Bearer ${token}` },
      });
